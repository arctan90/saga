...
from utils.file_util import load_audio
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, val=False):
        ...
        self.cnhubert_path = hparams.cnhubert_path
        self.wav32_path = hparams.wav32_path
        assert os.path.exists(self.bert_path)
        assert os.path.exists(self.cnhubert_path)
        assert os.path.exists(self.wav32_path)
        names4 = set([name[:-3] for name in list(os.listdir(self.cnhubert_path))])  # 去除.pt后缀
        names5 = set(os.listdir(self.wav32_path))
        self.phoneme_data = {}
        with open(self.bert_path, "r", encoding="utf8") as f:
            lines = f.read().strip("\n").split("\n")
        ...
        for audiopath in tqdm(self.audiopaths_sid_text):
            ...
            size = os.path.getsize("%s/%s" % (self.wav32_path, audiopath))
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        try:
            spec, wav = self.get_audio("%s/%s" % (self.wav32_path, audiopath))
            with torch.no_grad():
                ssl = torch.load("%s/%s.pt" % (self.cnhubert_path, audiopath), map_location="cpu")
